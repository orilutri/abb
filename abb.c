...
void *abb_borrar(abb_t *arbol, const char *clave) {
  nodo_t* padre = buscar_padre(arbol->raiz, clave, arbol->abb_comparar_clave, NULL);
...
void* buscar_padre(nodo_t *raiz, const char* clave, abb_comparar_clave_t abb_comparar_clave, nodo_t* padre) {
  if (!raiz) return NULL;
  if (abb_comparar_clave(clave, raiz->clave) < 0) return buscar_nodo(raiz->izq, clave, abb_comparar_clave, raiz);
  if (abb_comparar_clave(clave, raiz->clave) == 0) return padre;
  buscar_nodo(raiz->der, clave, abb_comparar_clave, raiz);
}
...
_abb_in_order(nodo_t* raiz, bool visitar(const char *, void *, void *), void *extra) {
  if (!raiz) return;
